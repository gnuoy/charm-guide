...
describes the charm and how it relates to other charms. This is also contrusted
from each layer that defines a metadata.yaml
...
This layer includes a a wheelhouse to pull in `charms.openstack <https://github.com/openstack/charms.openstack>`__
...
the charm handlers can call to run charm specfic code. 
...
complete the 'charm.installed' state is raised.  The charms handlers specfic
...
the **charm.install** event. To do this edit
...
Keystones `keystone interface <https://github.com/openstack/charm-interface-keystone>`__
...
If the charm is deploginy the Newton release, it first seraches
...
by examing the keys of the restart_map dict which was specified as part of the
charm class. Taking all this together results in a handler like this:
